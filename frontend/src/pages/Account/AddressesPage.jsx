...
async function reverseGeocodeLocation(latitude, longitude) {
  ...
  if (!response.ok) throw new Error('Không lấy được thông tin địa chỉ từ bản đồ')
...
export default function AddressesPage() {
  ...
  useEffect(() => {
    ...
    async function loadAccountData() {
      try {
        const [nextAddresses, nextProfile] = await Promise.all([getAccountAddresses(), getAccountProfile()])
        if (active) {
          setAddresses(nextAddresses)
          setProfile(nextProfile)
        }
      } catch (err) {
        if (active) toast.error(err.message || 'Không tải được thông tin địa chỉ')
      } finally {
  ...
  function openCreateModal() {
    ...
    setForm(profileContactReady ? { ...emptyAddressForm, ...profileContact } : emptyAddressForm)
    setModalOpen(true)
  }

  function openEditModal(address) {
    setEditingAddress(address)
    setContactMode(profileContactReady && addressUsesProfileContact(address, profile) ? 'profile' : 'custom')
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          toast.success(location.ward || location.province ? 'Đã lấy vị trí hiện tại.' : 'Đã lấy tọa độ hiện tại.')
        } catch (err) {
          setForm((current) => ({
            ...current,
            latitude,
          ...
          toast.warning(err.message || 'Đã lấy tọa độ, nhưng chưa tự điền được địa chỉ')
        } finally {
          setLocating(false)
        }
      },
    ...
    <AccountLayout>
      <section className="rounded-lg bg-surface-container-lowest px-6 py-6 shadow-sm md:px-8">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-outline-variant pb-4">
          <div>
            <h1 className="text-title-md font-title-md text-on-surface">Địa chỉ của tôi</h1>
            <p className="mt-1 text-body-sm text-on-surface-variant">Quản lý địa chỉ giao hàng và nhận hàng</p>
          </div>
          <button
        ...
        {!loading && !addresses.length ? (
          <div className="mt-5 rounded-lg border border-dashed border-outline-variant px-6 py-10 text-center">
            <span className="material-symbols-outlined text-[36px] text-primary">location_on</span>
            <p className="mt-3 text-title-sm font-title-sm text-on-surface">Chưa có địa chỉ</p>
        ...
        <div className="mt-5 space-y-4">
          {addresses.map((item) => (
            <article key={item.id} className="rounded-lg border border-outline-variant px-4 py-4">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="text-title-sm font-title-sm text-on-surface">{item.fullName}</p>
                    <span className="text-on-surface-variant">|</span>
                    <p className="text-body-sm text-on-surface-variant">{item.phone}</p>
                  </div>
                  <p className="mt-2 max-w-2xl text-body-sm text-on-surface-variant">{formatAddress(item)}</p>
                  ...
                  {item.isDefault ? (
                    <span className="mt-3 inline-flex rounded-md border border-primary px-2 py-1 text-label-md text-primary">
                      Mặc định
                    </span>
                  ) : null}
                </div>
                <div className="flex gap-3 text-label-md">
                  <button className="text-primary hover:underline" type="button" onClick={() => openEditModal(item)}>
                    Cập nhật
                  </button>
                  <button
          ...
          >
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <label className="grid gap-2">
                <span className="text-body-sm text-on-surface-variant">Tên người nhận</span>
                <input
              ...
              <label className="grid gap-2">
                <span className="text-body-sm text-on-surface-variant">Số điện thoại</span>
                <input
                  className="h-10 rounded-lg border-outline-variant bg-surface-container-low text-body-sm focus:border-primary focus:ring-primary disabled:text-on-surface-variant"
                  value={form.phone}
              ...
              <label className="grid gap-2 sm:col-span-2">
                <span className="text-body-sm text-on-surface-variant">Địa chỉ cụ thể</span>
                <input
              ...
              <div className="grid gap-3 rounded-lg border border-outline-variant px-4 py-4 sm:col-span-2">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p className="text-label-lg font-label-lg text-on-surface">Vị trí bản đồ</p>
                    <p className="mt-1 text-body-sm text-on-surface-variant">
